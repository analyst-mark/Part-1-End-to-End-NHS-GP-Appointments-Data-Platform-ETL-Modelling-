...
DECLARE @FolderPath NVARCHAR(500) = 
    N'C:\Users\mark\Desktop\Part-1-End-to-End-NHS-GP-Appointments-Data-Platform-ETL-Modelling-\datasets\2022\';
...
BEGIN
    ...
    -- Count before
    SELECT @Before = COUNT(*) FROM bronze.appointments;
    ...
    -- Rows inserted
    SELECT @After = COUNT(*) FROM bronze.appointments;
...
SELECT RIGHT(appointment_date, 4) AS Year,
       COUNT(*) AS TotalRows
FROM bronze.appointments
GROUP BY RIGHT(appointment_date, 4)
ORDER BY Year;
